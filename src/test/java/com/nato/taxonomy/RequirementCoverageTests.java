...
import com.nato.taxonomy.dto.CoverageStatistics;
import com.nato.taxonomy.repository.RequirementCoverageRepository;
import com.nato.taxonomy.service.RequirementCoverageService;
...
class RequirementCoverageTests {
    ...
    void recordingCoverageStoresEntriesAboveThreshold() {
        Map<String, Integer> scores = Map.of("CP-1", 85, "BP-3", 72, "BP-5", 30);
        coverageService.analyzeCoverage(scores, "REQ-001", "Some requirement text", 50);

        var forCp1 = coverageService.getCoverageForNode("CP-1");
        var forBp3 = coverageService.getCoverageForNode("BP-3");
        var forBp5 = coverageService.getCoverageForNode("BP-5");

        assertThat(forCp1).hasSize(1);
        assertThat(forCp1.get(0).getScore()).isEqualTo(85);
        assertThat(forBp3).hasSize(1);
        assertThat(forBp5).isEmpty(); // below threshold
    }

    @Test
    void recordingCoverageReplacesExistingEntries() {
        coverageService.analyzeCoverage(Map.of("CP-1", 80), "REQ-001", "text", 50);
        coverageService.analyzeCoverage(Map.of("CP-1", 90), "REQ-001", "text", 50);

        var entries = coverageService.getCoverageForNode("CP-1");
    ...
    void getCoverageForRequirementReturnsAllNodes() {
        coverageService.analyzeCoverage(Map.of("CP-1", 80, "BP-3", 70), "REQ-002", "another req", 50);
    ...
    void coverageStatisticsShowsCoveredNodes() {
        coverageService.analyzeCoverage(Map.of("CP-1", 80, "BP-3", 70), "REQ-003", "text", 50);
    ...
    void deleteCoverageRemovesAllEntriesForRequirement() {
        coverageService.analyzeCoverage(Map.of("CP-1", 80), "REQ-DEL", "text", 50);
    ...
    void densityMapContainsCoveredNodes() {
        coverageService.analyzeCoverage(Map.of("CP-1", 80, "BP-3", 70), "REQ-004", "text", 50);
        Map<String, Integer> density = coverageService.getRequirementDensityMap();
        assertThat(density).containsKey("CP-1");
        assertThat(density).containsKey("BP-3");
    }
    ...
    void recordEndpointStoresCoverage() throws Exception {
        String body = "{\"requirementId\":\"REQ-API-01\",\"requirementText\":\"api test\","
                + "\"scores\":{\"CP-1\":80,\"BP-3\":40},\"minScore\":50}";
        ...
        mockMvc.perform(get("/api/coverage/requirement/REQ-API-01").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.length()").value(1)) // only CP-1 passes threshold
                .andExpect(jsonPath("$[0].nodeCode").value("CP-1"));
    }

    @Test
    void deleteEndpointRemovesCoverage() throws Exception {
        coverageService.analyzeCoverage(Map.of("CP-1", 80), "REQ-API-DEL", "text", 50);
