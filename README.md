...
**13 concrete architecture elements** across **7 layers** (Capabilities, Communications Services, Core Services, COI Services, User Applications, Business Processes, Business Roles), connected by **18 traced relations** — including ⚠ impact hotspots and cross-layer dependency chains:
...
> **Mermaid graph vs. UI screenshots:** The Mermaid diagram above is a **compact showcase** generated by `MermaidExportService.exportShowcase()` — it selects the highest-impact leaf nodes per layer and limits edges for readability (15 elements, 7 layers, 12 relations). The UI screenshots below show a **full analysis run** using the live LLM, which scores _all_ ~2,500 taxonomy nodes and may include different element counts (typically 20+ elements, all 8 layers). The two views serve different purposes: the Mermaid graph communicates the _structure_ of the result; the UI shows the _complete_ scored tree and architecture view with all enrichment and propagation details.
