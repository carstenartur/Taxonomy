...
```xml
<jgit-storage-hibernate.version>0.1.12</jgit-storage-hibernate.version>
```
...
Version 0.1.12 is currently published through GitHub Packages. Maven credentials must use the same server ID as the repository entry in `pom.xml`:
...
Taxonomy contains no database-specific `ALTER TABLE` statements for the library-owned columns. It performs classification, preflight and post-migration validation; all physical adoption changes come from the immutable migration resources packaged in `jgit-storage-hibernate-core:0.1.12`.
...
Do not use Hibernate `ddl-auto=update`, manual ad-hoc DDL, Flyway `repair`, or deletion of migration history as substitutes for this procedure. Taxonomy never chooses a duplicate row or truncates an oversized value automatically. Upstream issue #78 is resolved by release 0.1.12.
