...
Open **Architecture editor** from Projects or the snapshot workbench. The context line shows repository, workspace, branch, commit and actor. Historical commits and central read contexts are read-only. Use **Create private workspace** when repository membership allows it; editing also requires the ARCHITECT or ADMIN role.
...
3. Enter a reason and choose **Preview change**. Review the affected blocks and displayed commit, then **Accept and commit**. Nothing is persisted by typing or previewing. **Discard draft** restores the accepted properties.
...
If a branch moves after preview, choose **Refresh, compare and preview again**, inspect the new before/after state and accept explicitly. A network failure retains the exact command identity; it must not be replaced by a blind new command. A Git commit can succeed while its secondary relation projection needs rebuilding: the accepted SHA remains authoritative, and **Rebuild projection** repairs that projection without another model change.
...
SVG and vector PDF use the complete deterministic server layout at the displayed commit. They are independent of local pan/zoom. The editor currently uses derived layout and a read-only synchronized DSL view; editable DSL translation, saved layouts, shared undo, richer evidence decisions and extended history remain tracked in [#814](https://github.com/carstenartur/Taxonomy/issues/814). Existing analysis snapshots remain immutable.
