...
function formatFindings(items, fatalError) {
  const lines = [];
  if (fatalError) {
    lines.push('Accessibility audit execution error:');
    lines.push(fatalError);
    lines.push('');
  }
  if (!items.length) {
    lines.push('No critical or serious axe violations found in the completed UI section audits.');
  } else {
...
try {
  ...
  auditError = error && error.stack ? error.stack : String(error);
  process.exitCode = 1;
} finally {
  const textReport = formatFindings(findings, auditError);
  await Promise.all([
    writeFile(reportPath, JSON.stringify({ baseUrl, auditError, findings }, null, 2) + '\n', 'utf8'),
    writeFile(textReportPath, textReport, 'utf8')
  ]);
  if (auditError || findings.length) {
