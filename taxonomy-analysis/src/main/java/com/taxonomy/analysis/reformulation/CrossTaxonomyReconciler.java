...
public class CrossTaxonomyReconciler {
    ...
    private ReformulationDocument reword(ReformulationBaseline baseline,ReformulationDocument before,ReconciliationResult review,List<DecisionAnswer> answers,Map<String,String> boundary,Set<String> touched,ReformulationStepExecutor steps) {
        ...
        for(String id:ordered) {
            ...
            var result=steps.execute("REWORD",in,NodeSynthesisResult.class,()->nodes.synthesize(in));result.statementProposals().forEach(s->statements.putIfAbsent(s.id(),s));result.questionProposals().forEach(q->questions.putIfAbsent(q.id(),q));
