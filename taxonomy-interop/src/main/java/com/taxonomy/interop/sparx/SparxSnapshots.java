...
public final class SparxSnapshots {
    ...
    public static boolean isSparx(String profile) { return SparxMappingProfile.PROFILE.equals(profile) || SparxOslcAmCodec.PROFILE.equals(profile); }
    ...
    public static ExchangeDocument identify(ExchangeDocument source, UUID connection) {
        ...
        List<Artifact> artifacts = source.artifacts().stream().map(a -> {
            Map<String, String> tags = tags(a.attributes(), connection, a.id(), identities, source);
        ...
        List<Relation> relations = source.relations().stream().map(r -> {
            Map<String, String> tags = tags(r.attributes(), connection, r.id(), identities, source);
    ...
    private static Map<String, String> tags(Map<String, String> before, UUID connection, String externalId, Set<String> identities, ExchangeDocument source) {
        ...
        result.put("tag:taxonomy.id", id); result.put("tag:taxonomy.mappingProfile", source.profile() + "@" + source.profileVersion());
        return result;
    }
}
