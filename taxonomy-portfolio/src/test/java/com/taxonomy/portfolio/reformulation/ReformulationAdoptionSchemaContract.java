...
public final class ReformulationAdoptionSchemaContract {
    ...
    private static String table(Connection c,String name) throws SQLException {return c.getMetaData().storesUpperCaseIdentifiers()?name.toUpperCase(Locale.ROOT):name;}
    private static void columns(Connection c,String name,List<String> expected) throws SQLException {
        ...
        try(var rows=c.getMetaData().getColumns(c.getCatalog(),c.getSchema(),table(c,name),null)) {
            while(rows.next()) {
                String column=rows.getString("COLUMN_NAME").toLowerCase(Locale.ROOT);actual.add(column);
                if(expected.contains(column) && rows.getInt("NULLABLE")!=DatabaseMetaData.columnNoNulls)throw new AssertionError("Nullable adoption column: "+column);
            }
        }
        if(!actual.containsAll(expected))throw new AssertionError("Missing adoption columns: "+name+" "+actual);
    }
    private static void fk(Connection c,String name,String key,Set<String> expected) throws SQLException {
        ...
        try(var rows=c.getMetaData().getImportedKeys(c.getCatalog(),c.getSchema(),table(c,name))) {
            while(rows.next())if(key.equalsIgnoreCase(rows.getString("FK_NAME")))actual.add((rows.getString("FKCOLUMN_NAME")+"->"+rows.getString("PKTABLE_NAME")+"."+rows.getString("PKCOLUMN_NAME")).toLowerCase(Locale.ROOT));
        }
        if(!actual.equals(expected))throw new AssertionError("Wrong adoption FK "+key+": "+actual);
    }
}
