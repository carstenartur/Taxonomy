...
- **Architecture intelligence** — scored analysis results are assembled into architecture views. Mermaid is available as a text projection; ArchiMate 3.1 and Visio 2012 downloads are experimental bounded subsets whose interoperability and desktop acceptance are tracked separately.
...
|---|---|
| **Open Standards** | The REST API uses OpenAPI; Mermaid, ONNX, and CycloneDX are open formats. ArchiMate output is an experimental bounded 3.1 subset, while the optional VSDX handoff is tracked separately as a proprietary interoperability surface. |
| **Open Source First** | MIT license. Full source code public. openCode-compatible. |
| **Interoperability** | REST API with an OpenAPI specification, a framework import pipeline (UAF, APQC, C4), and format-specific export boundaries documented in the feature matrix. |
...
flowchart TD
    ...
    G --> H1["Experimental ArchiMate 3.1 subset"]
    G --> H2["Experimental Visio 2012 subset"]
...
7. **Export** — the projected model is exported to the chosen format:
   - `ArchiMateDiagramService` → experimental bounded ArchiMate 3.1 subset (`.archimate` / `.xml`)
   - `VisioDiagramService` → experimental bounded Visio 2012 VSDX subset
   - `MermaidExportService` → Mermaid flowchart (`.md`)

The ordinary analysis export controls are not evidence of persisted-snapshot equivalence. Only an endpoint explicitly bound to a selected read-only Architecture Workbench snapshot carries that authority. See the [Architecture export support boundary](FEATURE_MATRIX.md#architecture-export-support-boundary); common artifact-envelope and authority evidence remain tracked in #966.
...
DSL documents are stored under the filename `architecture.taxdsl`. `DslApiController` owns Git commands, text diff, branching, merge and cherry-pick. `composition.dsl` owns export/current, materialization, archive-enriched history, structural/semantic comparison and document listing through `DslDocumentApiController` and `DslDocumentOperationsFacade`; Git authority and archive policy are unchanged. See [DSL document composition](../dev/DSL_DOCUMENT_COMPOSITION.md). Provenance blocks (`source`, `sourceVersion`, `sourceFragment`, `requirementSourceLink`) are stored in the same JGit DFS repository alongside architecture blocks.
...
When a `DslGitRepositoryFactory` is configured (default in production), each workspace
gets its own logically separate Git repository within the same database. Without the
factory, workspaces are isolated via branches in a shared repository.

### Repository-per-Workspace Architecture
...
└──────────┼───────────────────────────────────────────┘
           │
           │ fetch/push (EXTERNAL_CANONICAL mode)
           ▼
┌──────────────────────┐
│   Gitea / GitHub     │
│   Remote Repository  │
...
|---|---|
| **DslGitRepositoryFactory** | Creates and caches per-workspace `DslGitRepository` instances |
...
| **ExternalGitSyncService** | Fetch/push operations to external Git remote (EXTERNAL_CANONICAL mode) |

### Isolation Model

1. **Repository isolation** — When `DslGitRepositoryFactory` is configured, each workspace gets its own Git repository (same DB, different namespace). Without the factory, branch-based isolation is used.
2. **State isolation** — Navigation context, projection tracking, and operation state are per-user.
3. **Sync workflow** — Users pull from shared (sync) and push to shared (publish) explicitly. Cross-repository sync copies DSL content between workspace and system repositories.
...
Browser → WorkspaceResolver (extract user)
       ...
       → DslGitRepositoryFactory (resolve repo for workspace)
...
flowchart TB
    ...
    subgraph Output["Output Layer"]
        ...
        Visio["Experimental Visio 2012<br/>subset"]
        ArchiEx["Experimental ArchiMate 3.1<br/>subset"]
