...
`CUSTOM_LLM_URL` muss der vollständige HTTP- oder HTTPS-Endpunkt für Chat Completions sein, nicht nur ein Hostname oder eine API-Basis-URL. `CUSTOM_LLM_MODEL` wird unverändert in den Request übernommen.
...
|---|---|
| `CUSTOM_OPENAI` fehlt in der Providerauswahl | URL oder Modell ist leer, oder die URL ist keine gültige `http://`- beziehungsweise `https://`-URI |
...
Siehe auch [KI-Anbieter](AI_PROVIDERS.md) und die [Konfigurationsreferenz](CONFIGURATION_REFERENCE.md).
