#!/usr/bin/env python3
"""Correct stale testing, HSQLDB, persistence, and CI descriptions in EN/DE docs."""
...
```bash
# PostgreSQL diagnostics and browser tests
mvn -B -pl taxonomy-app \
  failsafe:integration-test failsafe:verify \
  -DskipITs=false \
...
`AbstractDatabaseContainerIT` or `AbstractSeleniumContainerIT`; the application
and database containers share a Testcontainers network and receive the selected
Spring profile/JDBC settings through environment variables.
...
Failsafe-/Testcontainers-Tests. Alle drei `db-*`-Tags sind jedoch im
Standard-Lebenszyklus ausgeschlossen. Relevante Pull Requests zur
Datenbankkonfiguration führen das begrenzte PostgreSQL-Paar aus Diagnose- und
Selenium-Test aus. Geplante und manuelle Workflows decken die ausgewählte
vollständige Matrix ab.

```bash
# PostgreSQL-Diagnose- und Browser-Tests
mvn -B -pl taxonomy-app \
  failsafe:integration-test failsafe:verify \
  -DskipITs=false \
  -Dit.test='*Postgres*IT' \
...
replace_once(
    ...
    "- **In-process HSQLDB** — the zero-configuration profile uses embedded HSQLDB; file-backed deployments persist the catalogue and JGit data, while an empty database or an explicit reload imports the bundled workbook. No external database is required by default.",
)
replace_once(
...
PostgreSQL, Microsoft SQL Server, and Oracle use their dedicated profiles and
bounded Testcontainers compatibility matrix. JPA nationalized/LOB mappings and
safe column names cover Unicode, large text, embeddings, and reserved-word
differences. Detailed setup and test commands are in
[Database Setup](DATABASE_SETUP.md) and
[`docs/dev/06-testing-by-change-type.md`](../dev/06-testing-by-change-type.md).''',
)
...
begrenzte Testcontainers-Kompatibilitätsmatrix. Nationalisierte/LOB-JPA-
Mappings und sichere Spaltennamen berücksichtigen Unicode, lange Texte,
Embeddings und reservierte Wörter. Details und Testbefehle stehen in
[Datenbank-Setup](DATABASE_SETUP.md) und
[`docs/dev/06-testing-by-change-type.md`](../dev/06-testing-by-change-type.md).''',
)
...
for path, old, new in [
    ...
    (
        "docs/de/ARCHITECTURE.md",
        "Beim Start lädt `TaxonomyService` den C3-Taxonomiekatalog aus der mitgelieferten Excel-Arbeitsmappe (`src/main/resources/data/C3_Taxonomy_Catalogue_25AUG2025.xlsx`) mit Apache POI. Ein CSV-Fallback (`relations.csv`) liefert Seed-Beziehungen, wenn in der Arbeitsmappe kein Relations-Blatt vorhanden ist.",
        "Wenn der Katalog leer ist oder `TAXONOMY_INIT_RELOAD_EXISTING=true` ausdrücklich einen Ersatz anfordert, importiert `TaxonomyService` die mitgelieferte C3-Arbeitsmappe (`src/main/resources/data/C3_Taxonomy_Catalogue_25AUG2025.xlsx`) mit Apache POI. Andernfalls werden persistierte Katalogzeilen weiterverwendet. Ein CSV-Fallback (`relations.csv`) liefert Seed-Beziehungen, wenn kein Relations-Blatt vorhanden ist.",
    ),
]:
