...
class KnowledgeModulePackagingIT {
    @Test
    void bootApplicationContainsOneKnowledgeLibraryAndNoDuplicateKnowledgeClasses() throws Exception {
        ...
        try (var jar = new ZipFile(applications.getFirst().toFile())) {
            ...
            var libraries = names.stream().filter(n -> n.startsWith("BOOT-INF/lib/taxonomy-knowledge-")
                    && n.endsWith(".jar")).toList();
            assertThat(libraries).hasSize(1);
            var ownedClasses = new HashSet<String>();
            var ownedResources = new HashSet<String>();
            var catalogueResources = List.of("data/C3_Taxonomy_Catalogue_25AUG2025.xlsx",
                    "data/nato-taxonomy.json", "data/relations.csv");
            for (String name : names) {
                ...
                try (var library = new ZipInputStream(jar.getInputStream(jar.getEntry(name)))) {
                    for (var entry = library.getNextEntry(); entry != null; entry = library.getNextEntry()) {
                        ...
                        if (catalogueResources.contains(path)) {
                            assertThat(name).as("library owning %s", path).isEqualTo(libraries.getFirst());
                            assertThat(ownedResources.add(path)).as("single occurrence of %s", path).isTrue();
                        }
                        if (path.endsWith(".class") && List.of("catalog", "relations", "search").stream()
            ...
            assertThat(ownedResources).containsExactlyInAnyOrderElementsOf(catalogueResources);
            for (String resource : catalogueResources) {
                assertThat(names).doesNotContain("BOOT-INF/classes/" + resource);
            }
            assertThat(ownedClasses).contains("com/taxonomy/catalog/service/TaxonomyService.class",
                    "com/taxonomy/relations/service/RelationTraversalService.class",
                    "com/taxonomy/search/EmbeddingBridgeSupport.class");
