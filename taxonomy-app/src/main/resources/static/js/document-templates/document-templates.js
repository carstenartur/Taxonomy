(function () {
    ...
    if (!api || !links) throw new Error('Document template support was not loaded');
    ...
    const text = german ? {
        ...
        current: 'Aktuell', unknown: 'Unbekannt', loading: 'Wird geladen…',
        ...
        current: 'Current', unknown: 'Unknown', loading: 'Loading…',
    ...
    function format(pattern, value) { return String(pattern).replace('{0}', String(value)); }
    function announce(message) {
        if (!liveRegion) return;
        liveRegion.textContent = '';
        window.setTimeout(function () { liveRegion.textContent = message; }, 0);
    }
    function hideAlerts() {
        errorAlert.classList.add('d-none'); successAlert.classList.add('d-none');
        errorAlert.textContent = ''; successAlert.textContent = '';
    }
    function showError(message) {
        successAlert.classList.add('d-none'); errorAlert.textContent = message;
        errorAlert.classList.remove('d-none'); errorAlert.focus({ preventScroll: false });
        announce(message);
    }
    function showSuccess(message) {
        errorAlert.classList.add('d-none'); successAlert.textContent = message;
        successAlert.classList.remove('d-none'); announce(message);
    }
    function apiUrl(path) { return new URL(apiBase + path, window.location.href).href; }
    function detailUrl(templateId) {
    ...
    function formatBytes(value) {
        ...
        if (!Number.isFinite(bytes) || bytes < 0) return text.unknown;
    ...
    function disableWordAction(anchor, reason) {
        anchor.removeAttribute('href'); anchor.classList.add('disabled');
        anchor.setAttribute('aria-disabled', 'true'); anchor.title = reason;
    }
    function configureWordAction(anchor, webDavUrl, mode) {
        if (!directWordEnabled) {
            disableWordAction(anchor, text.wordAuthenticationUnavailable); return;
        }
        if (links.directWordLinkAllowed(webDavUrl)) {
            links.configureWordLink(anchor, webDavUrl, mode); anchor.title = text.wordHint; return;
        }
        disableWordAction(anchor, text.wordHttpsRequired);
    }

    function renderTemplate(template) {
        const row = document.createElement('tr'); row.dataset.templateId = template.templateId;
        const identity = document.createElement('td');
        identity.append(element('div', 'fw-semibold', template.displayName),
            element('code', 'small d-block', template.templateId),
            element('span', 'small text-body-secondary', template.fileName + ' · '
                + formatBytes(template.uncompressedSize) + ' · ' + template.partCount
                + ' ' + text.ooxmlParts));
        const version = document.createElement('td');
        const commit = element('code', 'small d-block', String(template.headCommit).slice(0, 12));
        commit.title = template.headCommit;
        version.append(commit, element('span', 'small d-block', formatDate(template.updatedAt)),
        ...
        const actionGroup = element('div', 'd-flex flex-wrap gap-2');
        ...
        const copy = element('button', 'btn btn-sm btn-outline-secondary',
            labels.copy || 'Copy WebDAV address'); copy.type = 'button';
        copy.addEventListener('click', function () { copyAddress(webDavUrl); });
        const history = element('button', 'btn btn-sm btn-outline-secondary',
            labels.history || 'History'); history.type = 'button';
        history.addEventListener('click', function () { showHistory(template); });
        const manage = actionLink(text.manage, 'btn btn-sm btn-outline-primary');
        manage.href = detailUrl(template.templateId);
        actionGroup.append(download, copy, history, manage); actions.append(actionGroup);
        row.append(identity, version, word, actions); return row;
    }

    function renderTemplates(templates) {
        templatesById.clear(); rows.replaceChildren();
        const sorted = Array.from(templates || []).sort(function (a, b) {
    ...
    async function loadTemplates() {
        hideAlerts(); refreshButton.disabled = true;
        try {
            const templates = await api.list(apiUrl(''), text.loadFailed);
            renderTemplates(templates); announce(format(text.templatesLoaded, templates.length));
            return true;
        } catch (error) {
            renderTemplates([]); showError(error.message || text.loadFailed); return false;
    ...
    async function copyAddress(value) {
        try {
            if (navigator.clipboard && window.isSecureContext) {
                ...
                if (!copied) throw new Error(text.copyFailed);
            }
            showSuccess(text.copied);
        } catch (error) { showError(error.message || text.copyFailed); }
    }

    function setUploadBusy(busy) {
        uploadButton.disabled = busy; uploadSpinner.classList.toggle('d-none', !busy);
    ...
    uploadForm.addEventListener('submit', async function (event) {
        ...
        const nameInput = document.getElementById('documentTemplateDisplayName');
        const fileInput = document.getElementById('documentTemplateFile');
        idInput.value = idInput.value.trim().toLowerCase(); nameInput.value = nameInput.value.trim();
    ...
    async function showHistory(template) {
        ...
        historyBody.replaceChildren(element('p', 'text-body-secondary', text.loading)); modal.show();
        try {
            const revisions = await api.history(apiUrl('/' + encodeURIComponent(template.templateId)
                + '/history'), text.historyFailed);
            historyBody.replaceChildren();
            const manage = actionLink(text.manage, 'btn btn-primary btn-sm mb-3');
            manage.href = detailUrl(template.templateId); historyBody.appendChild(manage);
            if (!revisions.length) {
                historyBody.appendChild(element('p', 'text-body-secondary',
                    labels.historyEmpty || 'No versions are available.')); return;
            }
            ...
            revisions.forEach(function (revision, index) {
                ...
                const revisionCommit = element('code', '', String(revision.commitId).slice(0, 12));
                revisionCommit.title = revision.commitId;
                const badge = element('span', index === 0 ? 'badge text-bg-success'
                    : 'badge text-bg-secondary', index === 0 ? text.current
                    : formatDate(revision.committedAt));
                ...
                item.append(heading, element('div', 'mt-2', revision.message || text.unknown),
                    element('div', 'small text-body-secondary', (revision.author || text.unknown)
                        + ' · ' + formatDate(revision.committedAt)), historicalDownload);
    ...
    if (backLink && workspace.dataset.homeUrl) {
        ...
        home.hash = 'admin'; backLink.href = home.href;
    }
