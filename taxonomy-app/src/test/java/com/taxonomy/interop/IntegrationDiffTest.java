...
class IntegrationDiffTest {
    ...
    @Test void integrationMapsRejectNullEntriesWithStableValidation() {
        Map<String, String> attributes = new LinkedHashMap<>();
        attributes.put("nullable", null);
        var error = assertThrows(IllegalArgumentException.class,
                () -> new Artifact("requirement-1", ArtifactKind.REQUIREMENT, "type-1", "Title", "Body", attributes, Map.of()));
        assertEquals("Integration maps must not contain null keys or values", error.getMessage());
    }
