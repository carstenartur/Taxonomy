...
Only affected DSL blocks are formatted; unrelated source and ordering remain byte-identical. Comments and unknown properties of edited blocks are retained; comment placement, indentation and line endings may be normalized. Duplicate identities/properties and unterminated edited blocks fail closed. The import parser remains tolerant, with a brace-depth correction for full-line comments. Unknown existing extension fields and taxonomy references are read-only in the editor.
