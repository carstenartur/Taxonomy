...
set storage_repository_name = case
    ...
    owner_id = case when primary_repo then 'system' else coalesce(owner_id, 'system') end,
    created_by = case when primary_repo then 'system' else coalesce(created_by, 'system') end,
...
set source_branch = coalesce(source_branch, base_branch, sync_target_branch, 'draft'),
    relationship_type = coalesce(
        relationship_type,
        case when shared then 'INDEPENDENT' else 'WORKING_COPY' end),
