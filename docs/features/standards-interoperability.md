...
If a removed visual parent leaves a surviving child, `VIEW_OCCURRENCE_REPARENTED`
reports that the child moved to the view root and its retained coordinates need
layout review.
...
Archi 5.10.0 converts the fixture's straight-line attachment pairs into midpoint
bendpoints. `ARCHI_ATTACHMENTS_TO_BENDPOINT` records this observed product layout
transformation per connection; it is not described as lossless geometry. Unknown
coordinate changes fail the product gate. Taxonomy's own exchange round trip retains
the original points.

The [retained compatibility run and acceptance map](../qa/standards-interoperability.md)
record actual installed-product results and their exact source revisions.
...
vendor compatibility is not implied; each new head must pass the required job.
Core tests also exercise durable rollback/retry, three-way conflicts,
