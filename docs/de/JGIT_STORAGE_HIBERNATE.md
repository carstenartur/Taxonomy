...
Version 0.1.13 wird derzeit über GitHub Packages bereitgestellt. Die Maven-Zugangsdaten müssen dieselbe Server-ID wie der Repository-Eintrag in `pom.xml` verwenden:
...
Hibernate `ddl-auto=update`, manuelle Ad-hoc-DDL, Flyway `repair` oder das Löschen der Migrationshistorie dürfen diesen Ablauf nicht ersetzen. Taxonomy wählt niemals automatisch eine doppelte Zeile aus und kürzt keinen zu langen Wert. Das Upstream-Issue #78 ist durch Release 0.1.13 erledigt.
