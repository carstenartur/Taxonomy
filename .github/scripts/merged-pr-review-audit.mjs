...
export function mergedPullRequestSearchQuery(repository, sinceDate) {
    return [
        `repo:${repository}`,
        'is:pr',
        'is:merged',
...
export function isAuditableMergedPullRequest(pullRequest) {
    return Boolean(pullRequest?.merged_at ?? pullRequest?.mergedAt);
}
...
function reviewEvidence(review, changedFiles, headSha) {
...
function isCleanApprovalEvidence(evidence) {
    return evidence.exactHead
        && evidence.classification === 'approval-recommended'
...
export function auditMergedPullRequest({
    ...
    for (const review of lateReviews) {
        const evidence = reviewEvidence(review, changedFiles, headSha);
        if (!evidence.exactHead) {
            findings.push(finding('medium', 'STALE_HEAD_REVIEW_SUBMITTED_AFTER_MERGE',
                `A trusted review of non-merged head ${evidence.commit || 'unknown'} was submitted after merge.`,
                evidence));
            continue;
        }
        const actionable = !isCleanApprovalEvidence(evidence);
        const laterCleanReview = exactHeadReviews
            .filter(candidate => reviewSubmittedAt(candidate) > evidence.submittedAt)
            .map(candidate => reviewEvidence(candidate, changedFiles, headSha))
            .find(isCleanApprovalEvidence);
        const remediated = actionable && Boolean(laterCleanReview);
        findings.push(finding(actionable && !remediated ? 'high' : 'medium',
            actionable
                ? remediated
                    ? 'ACTIONABLE_REVIEW_REMEDIATED_AFTER_MERGE'
                    : 'ACTIONABLE_REVIEW_SUBMITTED_AFTER_MERGE'
                : 'APPROVING_REVIEW_SUBMITTED_AFTER_MERGE',
            actionable
                ? remediated
                    ? `A trusted exact-head review submitted after merge was followed by a complete, comment-free exact-head approval at ${laterCleanReview.submittedAt}.`
                    : `A trusted ${evidence.classification} exact-head review was submitted at ${evidence.submittedAt}, after merge at ${mergedAt}.`
                : `A complete, comment-free exact-head approval was submitted at ${evidence.submittedAt}, after merge at ${mergedAt}.`,
            evidence));
    }
...
function auditResult(
    ...
    return {
        number,
        title: singleLine(pullRequest?.title),
...
function highFindingMarker(headSha, code) {
    return `${COMMENT_MARKER}${headSha}:${markerCode(code)} -->`;
}

export function missingHighFindings(result, comments) {
    const trustedBodies = (comments ?? [])
        .filter(comment =>
            normalizeLogin(comment?.user?.login) === 'github-actions')
        .map(comment => String(comment?.body ?? ''));
    return findingsAtOrAbove(result?.findings, 'high')
        .filter(item => {
            const marker = highFindingMarker(result?.headSha, item.code);
            return !trustedBodies.some(body => body.includes(marker));
        });
}
...
class GitHubClient {
    ...
    async reviewThreads(number) {
        ...
        throw new Error(
            `Review-thread pagination exceeded 1,000 entries for PR #${number}.`);
    }

    async searchMergedPullRequests(sinceDate) {
        const query = mergedPullRequestSearchQuery(this.repository, sinceDate);
        const result = [];
        for (let page = 1; page <= 10; page++) {
...
function renderMarkdown(results, scope) {
    ...
    for (const result of results.filter(item => item.findings.length)) {
        ...
        lines.push('');
        lines.push(`- Base branch: \`${result.baseRef || 'unknown'}\``);
...
function auditComment(result, findings) {
    const markers = findings
        .map(item => highFindingMarker(result.headSha, item.code))
        .join('\n');
...
The repository-owned audit found high-priority review evidence for merged head \`${result.headSha}\` on base branch \`${result.baseRef || 'unknown'}\` that arrived too late, remained unresolved, or was not followed by a clean exact-head re-review:
...
async function commentOnce(client, result) {
    ...
    await client.addIssueComment(result.number, auditComment(result, missing));
    return true;
}
...
async function run() {
    ...
    const scope = requestedNumber
        ...
        : `merged PRs in repository since ${sinceDate}`;
    const report = {
        schemaVersion: 2,
    ...
    const commentable = results.filter(result =>
        findingsAtOrAbove(result.findings, 'high').length);
