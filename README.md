...
The result spans **8 architecture layers** with traceable relations, ⚠ impact hotspots (≥ 80%), and exportable diagrams. The graph below was **generated by the real pipeline** — not hand-crafted.

**10 concrete leaf-level architecture elements** across **8 layers** (Capabilities, Communications Services, Core Services, COI Services, User Applications, Business Processes, Business Roles, Information Products) — including all layers from the scoring trace above — connected by **10 traced relations**:

```mermaid
flowchart TD
    subgraph Capabilities["🔵 Capabilities"]
        CP_1023(["Communication and Info System Cap…<br/>★ ⚠ 85%"])
    end
    subgraph Business_Processes["🟢 Processes"]
        BP_1490["Health Services<br/>58%"]
        BP_1697["Medical Command, Control And Comm…<br/>52%"]
    end
    subgraph Business_Roles["🟢 Roles"]
        BR_1334["CIS Coordination and Advice Roles<br/>32%"]
    end
    ...
    subgraph Information_Products["🔷 Info Products"]
        IP_2106["CIS Coordination<br/>38%"]
    end
    ...
    CR_1047 -->|supports| BR_1334
    CR_1047 -->|depends on| CI
    UA_1574 -->|supports| BP_1697
    BP_1490 -->|uses| IP_2106
    CO_1063 -->|supports| CR_1047
...
> **Mermaid graph vs. UI screenshots:** The Mermaid diagram above is a **curated leaf-level showcase** — it shows only the most specific (leaf) nodes per layer and limits edges for readability (10 elements, 8 layers, 10 relations). Root and intermediate nodes (e.g., CP-1000, CO-1000) are omitted to keep the graph focused on concrete architecture elements. The UI screenshots below show a **full analysis run** using the live LLM, which scores _all_ ~2,500 taxonomy nodes and may include different element counts (typically 20+ elements, all 8 layers). The two views serve different purposes: the Mermaid graph communicates the _structure_ of the result; the UI shows the _complete_ scored tree and architecture view with all enrichment and propagation details.
