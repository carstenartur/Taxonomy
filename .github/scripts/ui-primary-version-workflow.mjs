...
export async function runVersionComparisonWorkflow({ page, role, evidence }) {
  ...
  const versions = await page.evaluate(async () => {
    ...
    const branch = 'qa-compare-' + Date.now();
  ...
  // The two commits live on a QA-only branch. No restore, active-model change or real LLM call.
  evidence.passed('Real relation-only version comparison, correct restore direction and visible failures');
}
