...
try {
const nodes = await api().searchTaxonomy(query);
...
if (!projectId) return;
try {
const portfolio = await api().getProjectPortfolio(projectId);
...
try {
await api().updateConflict(projectId, currentConflictButton.dataset.conflictId, {
status: status,
resolutionNote: resolution.value.trim() || null
});
