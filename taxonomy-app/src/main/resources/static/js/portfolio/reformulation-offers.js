...
(function () {
    ...
    window.TaxonomyPortfolioApi.listReformulations(Number(match[1]), Number(match[2])).then(function (offers) {
        ...
        offers.forEach(function (summary) {
            const details = element('details', undefined, 'border rounded p-3 mb-2');
            details.append(element('summary', labels.details + ' · ' + labels.revision + ' ' + summary.currentRevision));
            details.append(element('p', labels.state, 'fw-semibold mt-2'));
            details.append(element('p', labels.source + ' ' + summary.sourceVersionId + ' · Snapshot ' + summary.snapshotId, 'text-break'));
            ...
            details.addEventListener('toggle', async function () {
                ...
                try {
                    const offer = await window.TaxonomyPortfolioApi.getReformulation(Number(match[1]), Number(match[2]), summary.id);
                    content.replaceChildren(element('p', labels.unevaluated, 'text-body-secondary'));
                    ...
                    content.append(row, element('h3', labels.questions, 'h6 mt-3'));
                    if (!offer.currentRevision.questions.length) content.append(element('p', labels.noQuestions));
                    offer.currentRevision.questions.forEach(function (question) {
                        content.append(element('p', question.wording + ' (' + question.state + ')'));
                    });
                    loaded = true;
                } catch (failure) {
                    // Closing and reopening retries only this offer; other cards remain usable.
                    content.textContent = labels.failure;
                } finally {
                    loading = false;
