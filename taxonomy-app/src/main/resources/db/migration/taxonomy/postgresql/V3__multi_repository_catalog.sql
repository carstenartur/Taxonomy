...
create table repository_membership (
    id bigint generated by default as identity primary key,
    repository_id varchar(255) not null,
    username varchar(255) not null,
    repository_role varchar(255) not null,
    created_at timestamp(6) with time zone not null,
    created_by varchar(255) not null,
    updated_at timestamp(6) with time zone,
    version bigint not null default 0,
    constraint uq_repository_membership_repo_user
        unique (repository_id, username),
    constraint fk_repository_membership_repository
        foreign key (repository_id)
        references system_repository (repository_id)
        on delete cascade
);

create index idx_repository_membership_user
    on repository_membership (username);

create index idx_repository_membership_repository
    on repository_membership (repository_id);
