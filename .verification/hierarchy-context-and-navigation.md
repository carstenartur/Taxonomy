...
The existing category/product prompt builder and relationship adapter use this same method. Remote calls start only after scalar catalogue reads return. There are no additional model calls and no new provider. More complete context can increase prompt tokens; the existing prompt-budget guard still applies, and oversized context is not silently summarized away.
